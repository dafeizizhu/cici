...
class VCSDAO {
  ...
  findVCSListByUserId (userId) {
    return Sqlite3Driver.getSharedInstance().all(`SELECT * FROM vcs WHERE user_id = ?`, userId)
  }
  createVCS ({ type, description, username, password, user_id }) {
    return Sqlite3Driver.getSharedInstance().run(`INSERT INTO vcs (type, description, username, password, user_id) VALUES (?, ?, ?, ?, ?)`, [
  ...
  updateVCS ({ id, type, description, username, password, user_id }) {
    return Sqlite3Driver.getSharedInstance().run(`UPDATE vcs SET type = ?, description = ?, username = ?, password = ?, user_id = ? WHERE id = ?`, [
      type, description, username, password, user_id, id
    ])
  }
  removeVCS (id) {
    return Sqlite3Driver.getSharedInstance().run(`DELETE FROM vcs WHERE id = ?`, id)
  }
}
