...
module.exports = compose([
  ...
  _.get('/api/findAdmin', async ctx => {
    ctx.body = await ServerApi.getSharedInstance().findAdmin(ctx.query.userId)
  }),
  _.post('/api/saveAdminProject', async ctx => {
    ctx.body = await ServerApi.getSharedInstance().saveAdminProject(ctx.request.body.userId, ctx.request.body.projectIdList)
  })
])
