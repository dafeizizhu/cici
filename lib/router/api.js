...
module.exports = compose([
  ...
  _.get('/api/findCommits', async ctx => {
    ctx.body = await ServerApi.getSharedInstance().findCommits(ctx.query.branchId, ctx.session)
  }),
  _.post('/api/fetchCommits', async ctx => {
    ctx.body = await ServerApi.getSharedInstance().fetchCommits(ctx.request.body.branchId, ctx.session)
  })
])
